<template>
    <form @submit.prevent="submit" class="flex flex-col space-y-2">
        <FieldDatePicker v-model="form.date" :value="form.date" name="date" label="Izaberi datum" :error="form?.errors?.date"></FieldDatePicker>
        ...
        <FieldTextarea v-model="form.description" name="description" label="Dodatne informacije" :error="form?.errors?.description" class="sm:col-span-2" />
