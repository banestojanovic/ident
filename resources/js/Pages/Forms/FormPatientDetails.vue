<template>
    <form @submit.prevent="submit" class="space-y-2">
        ...
        <div class="flex items-end">
            <button type="button" @click="$emit('delete' )" class="mr-auto text-red-500 hover:underline">Izbriši pacijenta</button>
            <button
                type="submit"
                :class="[
                    loading ? 'opacity-25' : '',
...
const submit = () => {
    ...
    form.post(route("patients.update", {id: props.patient?.id}), {
        preserveScroll: true,
        onSuccess: () => {
            emit("success")
        },
