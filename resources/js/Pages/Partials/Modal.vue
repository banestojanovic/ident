    ...
    <TransitionRoot as="template" :show="open">
        <Dialog as="div" class="fixed inset-0 z-10 overflow-y-auto" @close="$emit('close')">
            <div class="flex min-h-screen items-center justify-center px-4 pb-20 pt-4 text-center sm:block sm:p-0">
                ...
                >
                    <div :class="[{'lg:max-w-3xl': type === 'big'}, 'relative inline-flex w-full transform rounded-lg bg-white p-6 text-left transition-all sm:max-w-xl sm:p-8 sm:align-middle']">
                        <button type="button" class="absolute right-4 top-6 text-sm uppercase text-red-500 sm:right-8 sm:top-8" @click="$emit('close')">Zatvori</button>
                        <div class="w-full">
                            <div class="flex flex-col">
                                <DialogTitle v-if="title" as="h3" class="flex items-center space-x-2 text-xl font-medium leading-6 text-gray-900">
