<template>
    <div class="overflow-hidden bg-white shadow sm:rounded-md">
        ...
        <ul role="list" class="divide-y divide-gray-200">
            <li v-for="record in patient?.records" :key="record.id">
                <div class="grid grid-cols-1 px-4 py-4 sm:grid-cols-3 sm:px-6 lg:grid-cols-6">
                    <div class="mt-2 sm:mt-0">
                        <p class="truncate text-lg font-medium">
                            {{ new Date(record?.date).toLocaleDateString(serbianLocale, options) }}
                        </p>
                    </div>
                    <div class="mt-2 sm:mt-0">
